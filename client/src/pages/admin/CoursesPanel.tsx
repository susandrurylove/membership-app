  ...
  return (
    <section>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="eyebrow">Structured learning</p><h2 className="mt-2 font-serif text-3xl text-[#172044]">Courses</h2></div>
        <Dialog open={createOpen} onOpenChange={setCreateOpen}><DialogTrigger asChild><Button className="rounded-full bg-[#0e1634] px-6 text-white"><Plus className="mr-2 size-4" /> New course</Button></DialogTrigger><CourseDialog media={media.data || []} onComplete={id => { setCreateOpen(false); setSelectedCourseId(id); }} /></Dialog>
      </div>
        ...
        {courses.error || media.error ? <QueryErrorState title="Course administration could not be loaded" onRetry={() => { void courses.refetch(); void media.refetch(); }} /> : courses.isLoading ? <Skeleton className="h-80 rounded-[1.5rem]" /> : courses.data?.length ? (
          <div className="grid gap-4 lg:grid-cols-2">
            {courses.data.map(row => (
              <article key={row.course.id} className="rounded-[1.5rem] border border-[#ddd5c5] bg-white p-6 shadow-[0_12px_35px_rgba(23,32,68,0.04)]">
                ...
                <h3 className="mt-5 font-serif text-3xl text-[#172044]">{row.course.title}</h3>
                ...
                <div className="mt-6 flex flex-wrap gap-3"><Button className="rounded-full bg-[#0e1634]" onClick={() => setSelectedCourseId(row.course.id)}><Settings2 className="mr-2 size-4" /> Manage lessons</Button><Dialog><DialogTrigger asChild><Button variant="outline" className="rounded-full bg-white"><Pencil className="mr-2 size-4" /> Edit course</Button></DialogTrigger><CourseDialog record={row.course} media={media.data || []} /></Dialog></div>
              </article>
            ))}
          </div>
...
function CourseEditor({ courseId, media, onBack }: { courseId: number; media: any[]; onBack: () => void }) {
  ...
  return (
    <section>
      ...
      <div className="mt-4 flex flex-col gap-5 rounded-[1.75rem] bg-[#0e1634] p-7 text-[#f5edd6] sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold tracking-[0.2em] text-[#c9a84c] uppercase">Course editor</p><h2 className="mt-2 font-serif text-4xl">{course.title}</h2><p className="mt-2 text-sm text-[#c9cbd4]">{lessons.length} lessons · {sections.length} sections · {course.status}</p></div><div className="flex flex-wrap gap-3"><Dialog open={sectionOpen} onOpenChange={setSectionOpen}><DialogTrigger asChild><Button variant="outline" className="rounded-full border-white/20 text-white"><Layers3 className="mr-2 size-4" /> Add section</Button></DialogTrigger><SectionDialog courseId={courseId} onComplete={() => setSectionOpen(false)} /></Dialog><Dialog open={lessonOpen} onOpenChange={setLessonOpen}><DialogTrigger asChild><Button className="rounded-full bg-[#c9a84c] text-[#0e1634] hover:bg-[#ddc36f]"><Plus className="mr-2 size-4" /> Add lesson</Button></DialogTrigger><LessonDialog courseId={courseId} sections={sections} media={media} onComplete={() => setLessonOpen(false)} /></Dialog></div></div>

      <div className="mt-7 space-y-5">
        {sections.map(section => {
          const sectionLessons = lessons.filter(lesson => lesson.sectionId === section.id);
          return <article key={section.id} className="rounded-[1.5rem] border border-[#ddd5c5] bg-white p-5 shadow-[0_12px_35px_rgba(23,32,68,0.04)]"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-bold tracking-[0.18em] text-[#8b6a19] uppercase">Section {section.sortOrder + 1}</p><h3 className="mt-1 font-serif text-2xl text-[#172044]">{section.title}</h3></div><Dialog><DialogTrigger asChild><Button variant="ghost" size="sm"><Pencil className="mr-2 size-4" /> Edit section</Button></DialogTrigger><SectionDialog record={section} courseId={courseId} /></Dialog></div><div className="mt-4 space-y-2">{sectionLessons.length ? sectionLessons.map(lesson => <LessonRow key={lesson.id} lesson={lesson} courseId={courseId} sections={sections} media={media} />) : <p className="rounded-xl bg-[#f7f2e8] px-4 py-3 text-sm text-muted-foreground">No lessons in this section yet.</p>}</div></article>;
        })}
        {unsectioned.length ? <article className="rounded-[1.5rem] border border-[#ddd5c5] bg-white p-5"><h3 className="font-serif text-2xl text-[#172044]">Unsectioned lessons</h3><div className="mt-4 space-y-2">{unsectioned.map(lesson => <LessonRow key={lesson.id} lesson={lesson} courseId={courseId} sections={sections} media={media} />)}</div></article> : null}
...
function LessonRow({ lesson, courseId, sections, media }: { lesson: any; courseId: number; sections: any[]; media: any[] }) {
  return <div className="flex flex-col gap-3 rounded-xl border border-[#ebe5d9] px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-lg bg-[#e7f0ec] text-[#246866]"><BookOpen className="size-4" /></div><div><p className="font-medium text-[#172044]">{lesson.title}</p><p className="mt-0.5 text-xs text-muted-foreground capitalize">{lesson.contentType} · {lesson.status} · order {lesson.sortOrder}</p></div></div><Dialog><DialogTrigger asChild><Button variant="ghost" size="sm"><Pencil className="mr-2 size-4" /> Edit</Button></DialogTrigger><LessonDialog record={lesson} courseId={courseId} sections={sections} media={media} /></Dialog></div>;
}
...
function SectionDialog({ record, courseId, onComplete }: { record?: any; courseId: number; onComplete?: () => void }) {
  ...
  return <DialogContent className="max-w-lg rounded-[1.5rem]"><DialogHeader><DialogTitle className="font-serif text-3xl">{record ? "Edit section" : "New section"}</DialogTitle><DialogDescription>Sections group related lessons into a clear path.</DialogDescription></DialogHeader><form className="mt-3 space-y-5" onSubmit={event => { event.preventDefault(); save.mutate({ id: record?.id, courseId, title, description: description || null, sortOrder: Number(sortOrder) || 0 }); }}><div className="space-y-2"><Label>Title</Label><Input required value={title} onChange={event => setTitle(event.target.value)} /></div><div className="space-y-2"><Label>Description</Label><Textarea value={description} onChange={event => setDescription(event.target.value)} /></div><div className="space-y-2"><Label>Order</Label><Input type="number" min="0" value={sortOrder} onChange={event => setSortOrder(event.target.value)} /></div>{save.error ? <p className="text-sm text-[#823b32]">{save.error.message}</p> : null}<div className="flex justify-end"><Button disabled={save.isPending} className="rounded-full bg-[#0e1634] px-6">{save.isPending ? <Loader2 className="size-4 animate-spin" /> : "Save section"}</Button></div></form></DialogContent>;
}
