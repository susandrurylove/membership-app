  ...
  return (
    <section>
      <div>
        ...
        <h2 className="mt-2 font-serif text-3xl text-[#243f4d]">Media library</h2>
      ...
      <div className="editorial-card mt-6 rounded-[1.5rem] p-5 sm:p-6">
      ...
      <div className="mt-7">
        {media.error ? <QueryErrorState title="The media library could not be loaded" onRetry={() => void media.refetch()} /> : media.isLoading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{[0, 1, 2].map(item => <Skeleton key={item} className="h-32 rounded-[1.5rem]" />)}</div> : media.data?.length ? (
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {media.data.map(asset => {
              const Icon = iconByKind[asset.kind];
              return (
                <article key={asset.id} className="editorial-card rounded-[1.5rem] p-5">
                  <div className="flex items-start gap-4"><div className="grid size-11 shrink-0 place-items-center rounded-xl bg-[#e7f0ec] text-[#246866]"><Icon className="size-5" /></div><div className="min-w-0"><p className="break-words font-medium text-[#243f4d]" title={asset.originalName}>{asset.originalName}</p><p className="mt-1 text-xs text-muted-foreground">{formatBytes(asset.byteSize)} · {new Date(asset.createdAt).toLocaleDateString()}</p></div></div>
