...
export function TeachingsPanel() {
  const qaDialog = import.meta.env.DEV ? new URLSearchParams(window.location.search).get("dialog") : null;
  const [createOpen, setCreateOpen] = useState(() => qaDialog === "teaching");
  const [categoryOpen, setCategoryOpen] = useState(() => qaDialog === "category");
  ...
  return (
    <section>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="eyebrow">Library publishing</p><h2 className="mt-2 font-serif text-3xl text-[#243f4d]">Teachings</h2></div>
        <div className="grid grid-cols-2 gap-3 sm:flex sm:flex-wrap">
          <Dialog open={categoryOpen} onOpenChange={setCategoryOpen}><DialogTrigger asChild><Button variant="outline" className="rounded-full bg-white px-3 sm:px-4"><FolderPlus className="mr-2 size-4" /> New category</Button></DialogTrigger><CategoryDialog onComplete={() => setCategoryOpen(false)} /></Dialog>
          <Dialog open={createOpen} onOpenChange={setCreateOpen}><DialogTrigger asChild><Button className="rounded-full bg-[#2f7772] px-3 text-white hover:bg-[#245f5c] sm:px-6"><Plus className="mr-2 size-4" /> New teaching</Button></DialogTrigger><TeachingDialog categories={categories.data || []} media={media.data || []} onComplete={() => setCreateOpen(false)} /></Dialog>
        </div>
      </div>
        ...
        {error ? <QueryErrorState title="Teaching administration could not be loaded" onRetry={() => { void teachings.refetch(); void categories.refetch(); void media.refetch(); }} /> : teachings.isLoading ? <Skeleton className="h-80 rounded-[1.5rem]" /> : teachings.data?.length ? (
          <div className="space-y-3">
            {teachings.data.map(row => (
              <article key={row.teaching.id} className="editorial-card flex flex-col gap-5 rounded-[1.5rem] p-5 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Badge className={row.teaching.status === "published" ? "bg-[#e7f0ec] text-[#246866] hover:bg-[#e7f0ec]" : row.teaching.status === "draft" ? "bg-[#f4ead0] text-[#8b6a19] hover:bg-[#f4ead0]" : "bg-[#eee] text-[#666] hover:bg-[#eee]"}>{row.teaching.status}</Badge><Badge variant="outline" className="capitalize">{row.teaching.contentType}</Badge>{row.teaching.featured ? <Badge variant="outline" className="border-[#c9a84c] text-[#8b6a19]">Featured</Badge> : null}</div><h3 className="mt-3 break-words font-serif text-2xl text-[#243f4d]">{row.teaching.title}</h3><p className="mt-1 text-xs text-muted-foreground">{row.category?.name || "Uncategorized"} · Updated {new Date(row.teaching.updatedAt).toLocaleDateString()}</p></div>
                <Dialog><DialogTrigger asChild><Button variant="outline" className="w-full shrink-0 rounded-full bg-white sm:w-auto"><Pencil className="mr-2 size-4" /> Edit</Button></DialogTrigger><TeachingDialog record={row.teaching} categories={categories.data || []} media={media.data || []} /></Dialog>
              </article>
            ))}
          </div>
...
function CategoryDialog({ onComplete }: { onComplete: () => void }) {
  ...
  return <DialogContent className="max-h-[90dvh] w-[calc(100%-2rem)] max-w-lg overflow-y-auto rounded-[1.5rem] p-5 sm:p-6"><DialogHeader className="pr-6"><DialogTitle className="font-serif text-2xl sm:text-3xl">New teaching category</DialogTitle><DialogDescription>Categories help members browse Susan’s library.</DialogDescription></DialogHeader><form className="mt-3 space-y-5" onSubmit={event => { event.preventDefault(); save.mutate({ name, description: description || null, sortOrder: 0 }); }}><div className="space-y-2"><Label htmlFor="category-name">Name</Label><Input id="category-name" required value={name} onChange={event => setName(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="category-description">Description</Label><Textarea id="category-description" value={description} onChange={event => setDescription(event.target.value)} /></div>{save.error ? <p role="alert" className="text-sm text-[#823b32]">{save.error.message}</p> : null}<div className="flex justify-end"><Button disabled={save.isPending} className="w-full rounded-full bg-[#2f7772] px-6 text-white hover:bg-[#245f5c] sm:w-auto">{save.isPending ? <Loader2 className="size-4 animate-spin" /> : "Create category"}</Button></div></form></DialogContent>;
}
...
function TeachingDialog({ record, categories, media, onComplete }: { record?: any; categories: any[]; media: any[]; onComplete?: () => void }) {
  ...
  return (
    <DialogContent className="max-h-[90dvh] w-[calc(100%-2rem)] max-w-3xl overflow-y-auto rounded-[1.5rem] p-5 sm:p-6">
      <DialogHeader className="pr-6"><DialogTitle className="font-serif text-2xl sm:text-3xl">{record ? "Edit teaching" : "New teaching"}</DialogTitle><DialogDescription>Use Markdown for headings, emphasis, links, and lists in the written body.</DialogDescription></DialogHeader>
      <form className="mt-3 space-y-5" onSubmit={event => { event.preventDefault(); save.mutate({ id: record?.id, title, summary: summary || null, body: body || null, categoryId: categoryId === "none" ? null : Number(categoryId), contentType, status, featured, sortOrder: Number(sortOrder) || 0, mediaAssetId: mediaAssetId === "none" ? null : Number(mediaAssetId) }); }}>
        ...
        <div className="flex justify-end"><Button disabled={save.isPending} className="w-full rounded-full bg-[#2f7772] px-7 text-white hover:bg-[#245f5c] sm:w-auto">{save.isPending ? <Loader2 className="size-4 animate-spin" /> : record ? "Save changes" : "Create teaching"}</Button></div>
      </form>
    </DialogContent>
  );
}
